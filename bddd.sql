...
CREATE PROCEDURE registrar_usuario(
    IN nombre CHAR(50),
    IN ap_paterno CHAR(50),
    IN ap_materno CHAR(50),
...
BEGIN
    ...
    IF NOT EXISTS (
        ...
        INSERT INTO usuarios (ID_usuario, Nombre, AP_paterno, AP_materno, Correo, Contrasena, Fecha_registro)
        VALUES (
            (SELECT COALESCE(MAX(ID_usuario), 0) + 1 FROM usuarios),
            nombre, ap_paterno, ap_materno, correo, contrasena, CURDATE()
        );
    ELSE
...
END$$
DELIMITER ;
CALL registrar_usuario('Maria', 'Martinez', 'Fernandez', 'maria.martinez@email.com', 'password456');
...
FOR EACH ROW
BEGIN
    INSERT INTO sesiones_de_usuario (ID_sesion, ID_usuario, Token_sesion, Fecha_inicio, Fecha_final)
    VALUES (
        (SELECT COALESCE(MAX(ID_sesion), 0) + 1 FROM sesiones_de_usuario),
        OLD.ID_usuario,
        CONCAT('Cambio-', UUID()),
        NOW(),
        NULL
